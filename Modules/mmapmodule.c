...
#ifdef	_UNICODE
		TCHAR Local_TCHAR_tagname[MAX_PATH+1];

		MultiByteToWideChar(CP_ACP, 0,
				    self->tagname, -1,
				    Local_TCHAR_tagname, sizeof(Local_TCHAR_tagname)/sizeof(Local_TCHAR_tagname[0]));
		TCHAR_tagname = Local_TCHAR_tagname;
#else	/* not _UNICODE */
		TCHAR_tagname = self->tagname;
#endif	/* not _UNICODE */
...
#else	/* MS_WINCE */
		/*
		 *	Windows/CE can't duplicate file handles
		 *
		 *	(Unfortunately, this means that Python isn't allowed to close the file)
		 */
