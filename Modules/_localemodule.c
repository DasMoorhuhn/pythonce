...
#define LOCALE_MAXLEN 100

static PyObject*
PyLocale_getdefaultlocale(PyObject* self)
{
    char encoding[100];
    TCHAR locale[LOCALE_MAXLEN];

    PyOS_snprintf(encoding, sizeof(encoding), "cp%d", GetACP());

    if (GetLocaleInfo(LOCALE_USER_DEFAULT,
                      LOCALE_SISO639LANGNAME,
                      locale, LOCALE_MAXLEN)) {
        int i = _tcslen(locale);
        locale[i++] = '_';
        if (GetLocaleInfo(LOCALE_USER_DEFAULT,
                          LOCALE_SISO3166CTRYNAME,
                          locale+i, LOCALE_MAXLEN - i))
            return Py_BuildValue(PYBUILD_TSTR "s", locale, encoding);
    }
