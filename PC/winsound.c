...
{
    const char *sound_arg;
    TCHAR *sound;
    int flags;
    int length;
    int ok;
#ifdef	_UNICODE
    TCHAR Local_Sound[64];
#endif	/* _UNICODE */


    if(!PyArg_ParseTuple(args,"z#i:PlaySound",&sound_arg,&length,&flags)) {
        return NULL;
    }

#ifdef	_UNICODE
    MultiByteToWideChar(CP_ACP, 0, sound_arg, -1, Local_Sound, sizeof(Local_Sound)/sizeof(Local_Sound[0]));
    sound = Local_Sound;
#else	/* not _UNICODE */
    sound = sound_arg;
#endif	/* not _UNICODE */
