...
#ifdef	_UNICODE
/*
 *	Convert a char * to a TCHAR * (return it in a static buffer)
 */
static TCHAR *TCHAR_From_String(char *String)
{
	char *cp;
	int Length;
	static TCHAR *Local_Buffer = 0;
	static int Local_Buffer_Size = 0;

	/*
	 *	A null string is a null string
	 */
	...
	if (Length > Local_Buffer_Size) {
		 ...
		 */
		if (Local_Buffer) LocalFree(Local_Buffer);
		Local_Buffer_Size = 0;
		Local_Buffer = (TCHAR *)LocalAlloc(0, Length * sizeof(TCHAR));
		if (!Local_Buffer) return(TEXT(""));
		Local_Buffer_Size = Length;
	}
	/*
	 *	Convert to wide string
	 */
	MultiByteToWideChar(CP_ACP, 0, String, -1, Local_Buffer, Local_Buffer_Size);
	return(Local_Buffer);
}
/*
 *	Convert a TCHAR * into a char * (in place)
 */
static void String_From_TCHAR(TCHAR *Wide_String)
{
	TCHAR *cp;
	char *cp1;

	cp = Wide_String;
	cp1 = (char *)Wide_String;
	while((*cp1++ = (char)*cp++) != '\0') ;
}
#else	/* not _UNICODE */
#define	TCHAR_From_String(String) String
#define	String_From_TCHAR(String)
#endif	/* not _UNICODE */
...
PyHKEY_printFunc(PyObject *ob, FILE *fp, int flags)
{
	PyHKEYObject *pyhkey = (PyHKEYObject *)ob;
	char resBuf[160];
	PyOS_snprintf(resBuf, sizeof(resBuf), "<PyHKEY at 0x%8.8X (0x%8.8X)>", ob, pyhkey->hkey);
	fputs(resBuf, fp);
	return 0;
}
...
PyHKEY_strFunc(PyObject *ob)
{
	PyHKEYObject *pyhkey = (PyHKEYObject *)ob;
	char resBuf[160];
	PyOS_snprintf(resBuf, sizeof(resBuf), "<PyHKEY:0x%8.8X>", pyhkey->hkey);
...
PyCreateKey(PyObject *self, PyObject *args)
{
	...
	if (!PyHKEY_AsHKEY(obKey, &hKey, FALSE))
		return NULL;
	rc = RegCreateKey(hKey, TCHAR_From_String(subKey), &retKey);
	if (rc != ERROR_SUCCESS)
...
PyDeleteKey(PyObject *self, PyObject *args)
{
	...
	if (!PyHKEY_AsHKEY(obKey, &hKey, FALSE))
		return NULL;
	rc = RegDeleteKey(hKey, TCHAR_From_String(subKey) );
	if (rc != ERROR_SUCCESS)
...
PyDeleteValue(PyObject *self, PyObject *args)
{
	...
	rc = RegDeleteValue(hKey, TCHAR_From_String(subKey));
...
PyEnumKey(PyObject *self, PyObject *args)
{
	...
	retStr = PyString_FromStringAndSize(NULL, len * sizeof(TCHAR));
	if (retStr == NULL)
		return NULL;
	retBuf = PyString_AS_STRING(retStr);

	if ((rc = RegEnumKey(hKey, index, (TCHAR *)retBuf, len)) != ERROR_SUCCESS) {
		Py_DECREF(retStr);
		return PyErr_SetFromWindowsErrWithFunction(rc, "RegEnumKey");
	}
	String_From_TCHAR((TCHAR *)retBuf);
...
PyEnumValue(PyObject *self, PyObject *args)
{
	...
	++retDataSize;
	retValueBuf = (char *)PyMem_Malloc(retValueSize * sizeof(TCHAR));
	if (retValueBuf == NULL)
		return PyErr_NoMemory();
	...
	String_From_TCHAR((TCHAR *)retValueBuf);
...
PyLoadKey(PyObject *self, PyObject *args)
{
	...
	if (!PyHKEY_AsHKEY(obKey, &hKey, FALSE))
		return NULL;
	Py_BEGIN_ALLOW_THREADS
	rc = RegLoadKey(hKey, TCHAR_From_String(subKey), fileName );
	Py_END_ALLOW_THREADS
	if (rc != ERROR_SUCCESS)
...
PyOpenKey(PyObject *self, PyObject *args)
{
	...
	Py_BEGIN_ALLOW_THREADS
	rc = RegOpenKeyEx(hKey, TCHAR_From_String(subKey), res, sam, &retKey);
	Py_END_ALLOW_THREADS
	if (rc != ERROR_SUCCESS)
...
PyQueryValue(PyObject *self, PyObject *args)
{
	...
	if (!PyHKEY_AsHKEY(obKey, &hKey, FALSE))
		return NULL;
	if ((rc = RegQueryValue(hKey, TCHAR_From_String(subKey), NULL, &bufSize))
	    != ERROR_SUCCESS)
		return PyErr_SetFromWindowsErrWithFunction(rc,
							   "RegQueryValue");
	...
	if (retStr == NULL)
		return NULL;
	retBuf = PyString_AS_STRING(retStr);
	if ((rc = RegQueryValue(hKey, TCHAR_From_String(subKey), retBuf, &bufSize))
	    != ERROR_SUCCESS) {
		Py_DECREF(retStr);
		return PyErr_SetFromWindowsErrWithFunction(rc,
							   "RegQueryValue");
...
PyQueryValueEx(PyObject *self, PyObject *args)
{
	...
	if (!PyHKEY_AsHKEY(obKey, &hKey, FALSE))
		return NULL;
	if ((rc = RegQueryValueEx(hKey, TCHAR_From_String(valueName),
				  NULL, NULL, NULL,
				  &bufSize))
	    != ERROR_SUCCESS)
		return PyErr_SetFromWindowsErrWithFunction(rc,
	...
	if (retBuf == NULL)
		return PyErr_NoMemory();
	if ((rc = RegQueryValueEx(hKey, TCHAR_From_String(valueName), NULL,
...
*/
	Py_BEGIN_ALLOW_THREADS
	rc = RegSaveKey(hKey, TCHAR_From_String(fileName), pSA );
	Py_END_ALLOW_THREADS
	if (rc != ERROR_SUCCESS)
...
PySetValue(PyObject *self, PyObject *args)
{
	...
	if (obSubKey == Py_None)
		...
		if (PyString_AsStringAndSize(obSubKey, &str, &len) != 0) return NULL;
		subKey = (TCHAR *)LocalAlloc(0, (len + 1) * sizeof(TCHAR));
		if (!subKey) {
			PyErr_NoMemory();
			return NULL;
		}
		CopyMemory(subKey, TCHAR_From_String(str), (len + 1) * sizeof(TCHAR));
	}
	/* XXX - need Unicode support */
	if (PyString_AsStringAndSize(obStrVal, &str, &len) != 0) return NULL;
	Py_BEGIN_ALLOW_THREADS
	rc = RegSetValue(hKey, subKey, REG_SZ, (char *)TCHAR_From_String(str), (len + 1) * sizeof(TCHAR));
	Py_END_ALLOW_THREADS
	if (subKey) LocalFree(subKey);
	if (rc != ERROR_SUCCESS)
...
PySetValueEx(PyObject *self, PyObject *args)
{
	...
	if (!PyHKEY_AsHKEY(obKey, &hKey, FALSE))
		return NULL;
	len = strlen(valueName_Arg);
	valueName = (TCHAR *)LocalAlloc(0, (len + 1) * sizeof(TCHAR));
	if (!valueName) {
		PyErr_NoMemory();
		return(NULL);
	}
	CopyMemory(valueName, TCHAR_From_String(valueName_Arg), (len + 1) * sizeof(TCHAR));
