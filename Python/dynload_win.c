				    ...
				    const char *pathname, FILE *fp)
{
...

	{
		HINSTANCE hDLL = NULL;
#ifdef	MS_WINCE
		/* Just use LoadLibrary with the supplied path */
		WCHAR Wide_String[260];
 		MultiByteToWideChar(CP_ACP, 0, pathname, -1, Wide_String, sizeof(Wide_String)/sizeof(Wide_String[0]));
 		hDLL = LoadLibrary(Wide_String);
#else	/* not MS_WINCE */
		char pathbuf[260];
		LPTSTR dummy;
		...
		if (hDLL==NULL){
			...
			char theInfo[256 * sizeof(TCHAR)]; /* Pointer to error text
							      from system */
			...
			theLength = FormatMessage(
				...
				sizeof(theInfo)/sizeof(TCHAR), /* the buffer size */
			...
			if (theLength == 0) {
				...
				char Temp[sizeof(theInfo)/sizeof(theInfo[0])];
				/* Convert back to Ascii */
				WideCharToMultiByte(CP_ACP, 0, (TCHAR *)theInfo, theLength, Temp, theLength, NULL, NULL);
				memcpy(theInfo, Temp, theLength);
#endif	/* MS_WINCE && UNICODE */
				/* For some reason a \r\n
...
#if defined(MS_WINCE) && defined(UNICODE)
 		MultiByteToWideChar(CP_ACP, 0, funcname, -1, Wide_String, sizeof(Wide_String)/sizeof(Wide_String[0]));
 		p = GetProcAddress(hDLL, Wide_String);
